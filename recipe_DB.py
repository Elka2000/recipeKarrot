...
while (x > 0) :
	...
	if title == None: 
		...
		for a, i in zip(ing_amount,ingredtags):
			try:
				curR.execute('INSERT OR IGNORE INTO Rec_Ingredients_Keys (ingredient_key) VALUES (?)',(i.text,)) 
		...
		for words in instructtags:
			curR.execute('UPDATE Recipes_Main SET instructions=? WHERE recipe_id=?',(words.text,recipe_id[0]))
			connR.commit()
			print(words.text,1)
		...
		for k, v in zip(nutri_key,nutri_amount):
			...
			try:
				curR.execute('INSERT OR IGNORE INTO Rec_Nutrition_Keys (nutrition_key) VALUES (?)',(k.text,)) 
				connR.commit()
